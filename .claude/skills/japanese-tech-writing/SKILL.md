...
description: 日本語で技術書の章 / 草稿 / 記事 / 解説文を書くとき、または既存の日本語技術文を推敲・リライトするとき、あるいは `/japanese-tech-writing` として明示呼び出ししたときに使う。文章規範（整形・パラグラフライティング・論証の厳密さ・LLM 口調の排除・冗長排除）に照らして執筆／レビューする。
model: opus
---

# 日本語技術文書の文章規範

日本語で技術的な原稿（書籍の章、記事、解説文）を **書く・推敲する** ときの文章規範。本文そのものの執筆・レビューを担う。
...
草稿を書く・書き直すときに、下記「文章規範」を適用して文章を生成する。

### レビューモード（推敲）

既存文を下記規範に照らして点検し、**規範カテゴリ別** に指摘する。本体を勝手に全面書き換えしない（指摘ベース）。

- **全カテゴリを漏れなく点検する。** 一般論的な感想で済ませず、必ず「どの規範に反するか」を名指しする。
- 規範に **無い** 一般助言（例: です／である調の統一など、下記に記載のない事項）を足さない。本 skill の責務は下記規範の適用に限る。
- **特に見落としやすい点**（self-check）:
  - **LLM っぽい表現**: 「正面から扱う / 回収する」（正面から系）、「重要なのは〜である」「本章では〜を扱う」「まとめると」「要するに」（予告と総括）、「不可欠」「核心的」「多角的」「包括的」（空虚な形容）、「掘り下げる」「深掘りする」（空虚な動詞）を型として検出する。
  - **論証の厳密さ**: 複数要因の事象を単一原因に還元していないか、因果を主張して機構（なぜ）を省いていないか、「必ず」と断定していないか。
  - **整形**: 一文一行になっているか、ダッシュ（——）や中黒（・）の誤用、見出しがセリフ／手順になっていないか。

#### レビュー出力フォーマット

```markdown
## カテゴリ: <整形 / 段落と論証 / 論証の厳密さ / 読み手の負荷 / 視点と語り / 演出の抑制 / LLM っぽい表現 / 冗長 / 見出し / 読者への誠実さ>
- 箇所: 「<原文の引用>」
  - 違反規範: <該当する規範を一文で>
  - 修正案: <具体的な書き換え>
